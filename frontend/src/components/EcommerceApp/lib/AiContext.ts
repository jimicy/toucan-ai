...
export const GENERATE_NEW_PRODUCT_USER_QUERY = `Come up with only one new product idea for me, based on products you've seen so far. The products should belong in the same category. Print just the new product name followed by a new line and its description followed the exact format as the current descriptions. The product description should be similar to the ones before. Never use the word peacock.

The <product description> should be in the following format
"Malaysia traditional batik cotton fabric pattern featuring <pick nouns describing the subject>"
...
export function generateContextQuery(
  ...
  if (recentSystemMessage && recentSystemMessage.type === "product-catalog") {
    ...
    recentSystemMessage &&
    recentSystemMessage.type === "popular-items-analysis"
  ) {
    const data: PopularItemAnalysis = recentSystemMessage.data;

    let context =
      "Use this store's popular products and target age bucket demographic psychographic analysis to inform coming up with a new product idea. Do not say this back to me.\n";
